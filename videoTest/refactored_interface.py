...
class interface:
    def __init__(self):
        ...
        # O for attached video camera 
        # 1 for connected video camera
    ...
    def make_hand(self, image):
        ...
        return cmh, results, lod, image

    def process_hand(self, cmh, results, dsize, lod, image):
        ...
        if cmh is not None:
            ...
            self.update_interface(cmh, cmhl, image)
        else:
        ...
        self.image_creation(results, image, dsize, lod)
    ...
    def update_interface(self, cmh, cmhl, image):
    ...
    def image_creation(self, results, image, dsize, lod):
    ...
    # For modularity:
        # Every block tracking detector will need to hand processing video, hence this function is important
        # Within the process video function, it needs a function to
            # Process each frame with image adjustments
            # Develop a log of which blocks moved by processing the image in whatever method given
    def process_video(self):
        while self.go:
            dsize, image = self.process_image()

            cmh, results, lod, image = self.make_hand(image)

            image, log = self.process_hand(cmh, results, dsize, lod, image)

            self.board.build_activated(log, image)
